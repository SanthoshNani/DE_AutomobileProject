...
if spark.catalog.tableExists('cars_catalog.gold.dim_dealer'):
    ...
    df_sink = spark.sql('''
    SELECT 1 as dim_dealer_key, Dealer_ID, Dealer_ID
    FROM PARQUET.`abfss://silver@dataengcarsdatalake.dfs.core.windows.net/carsales`
    WHERE 1=0
    ''')
...
# Incremental RUN
if spark.catalog.tableExists('cars_catalog.gold.dim_dealer'):
    delta_tbl = DeltaTable.forPath(spark, 'abfss://gold@dataengcarsdatalake.dfs.core.windows.net/dim_dealer')
...
#Initial RUN
    ...
    df_final.write.format('delta')\
            .mode('overwrite')\
            .option('path', 'abfss://gold@dataengcarsdatalake.dfs.core.windows.net/dim_dealer')\
